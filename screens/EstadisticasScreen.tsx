...
export default function EstadisticasScreen({ navigation }: any) {
  ...
  const puntajeMaximo = puntajes.length > 0 ? Math.max(...puntajes.map((item) => item.puntaje)) : 0;
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.estadisticasBox}>
        <Text style={styles.estadistica}>Puntaje Total: {puntajeTotal} pts</Text>
        <Text style={styles.estadistica}>Puntaje Más Alto: {puntajeMaximo} pts</Text>
        <Text style={styles.estadistica}>Puntaje Promedio: {puntajePromedio.toFixed(2)} pts</Text>
      </View>
...
const styles = StyleSheet.create({
  container: {
    padding: 16,
    backgroundColor: "#fff",
    flex: 1,  // Para que la vista ocupe todo el espacio disponible
  },
  titulo: {
    fontSize: 24,
    fontWeight: "bold",
